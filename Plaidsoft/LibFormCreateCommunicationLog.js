...
module.exports.main = function (ffCollection, vvClient, response) {
  ...
  logger.info(
    "Start of the process LibFormCreateCommunicationLog at " + Date()
  );
  ...
  return result
    .then(function () {
      ...
      scheduledSendDateTime = ffCollection.getFormFieldByName(
        "SCHEDULEDSENDDATETIME"
      ).value;
      approvedToSend = ffCollection.getFormFieldByName("APPROVEDTOSEND");
      otherFields = ffCollection.getFormFieldByName(
        "OTHERFIELDSTOUPDATE"
      ).value;
      ...
      if (!emailType || !emailType.value) {
        ...
        emailType.value.toLowerCase() != "digest" &&
        emailType.value.toLowerCase() != "immediate send"
      ) {
        inputErrors++;
        inputErrorsMessage +=
      ...
      if (!body || !body.value) {
        inputErrors++;
        inputErrorsMessage +=
          "The BODY parameter was not supplied. This should be the body of the email. <br>";
      }
      ...
      if (dateCheck.isValid() == false) {
        ...
        var sendTime = momentTz(scheduledSendDateTime)
          .tz(timeZone)
          .format("LT");
      ...
      if (!approvedToSend || !approvedToSend.value) {
        ...
        approvedToSend.value.toLowerCase() != "yes" &&
        approvedToSend.value.toLowerCase() != "no"
      ) {
        inputErrors++;
        inputErrorsMessage +=
    ...
    .then(function () {
      ...
      return vvClient.forms
        .postForms(null, targetFields, communicationLogTemplateID)
    ...
    .then(function () {
      if (numForms == successes) {
        returnObj[0] = "Success";
        returnObj[1] =
          "The request to create communication log completed successfully.";
