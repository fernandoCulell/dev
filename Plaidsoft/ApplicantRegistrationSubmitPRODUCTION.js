...
module.exports.main = async function (ffCollection, vvClient, response) {
  /*
    ...
    Preconditions:
                    - Libraries: LibUserCreateNoEmailOption, LibFormUpdateorPostandRelateFormAsyncAwait, LibCopyFiles, LibEmailGenerateAndCreateCommunicationLog
                    - Form Templates: "Plaidsoft Account Registration" Record for Email Notification Template.
    ...
    Pseudo code: 
                    1. Name site and user accounts
                      1.1. Format for siteName is firstName_lastName
                      1.2. Check if Site Name is unique; append number to the siteName until is unique.
                    2. Create the Site with the unique siteName.
                    3. Create unique Group in format: siteName_Group
                    4. Create folders, assign permissions and copy files.
                      4.1. Check if the folder already exists
                      4.2. Create folder if doesn't exist
                      4.3. Get folder's current permissions settings
                      4.4. Delete every current member with permissions set in the folder
                      4.5. Set new permissions to the folder
                      4.6. Copy files into new folders
                    5. Create 3 users with user names in the format: siteName_Applicant, siteName_HR, siteName_Manager
                    ...
                    9. Send one welcome email with login information for all three accounts.
                    10. Wait for the creating folders and copying files asynchronous process to finish.
                    11. Return response.

    Date of Dev:   12/23/2020
    Last Rev Date: 08/02/2022
    ...
    08/02/2022 - Fernando Culell: General refactoring of the code, including its processing sequence.
    */
  ...
  async function setUpJobApplications(formDataList) {
      ...
      formDataList.map((formData) => {
        ...
        try {
          ...
          return vvClient.scripts
            ...
            .then((res) => checkDataIsNotEmpty(res, shortDescription))
            .then((res) => {
              jobApplicationId = res.data[2].revisionId;
            });
        } catch (error) {
          errorLog.push(error);
        }
  ...
  try {
    ...
    // SET UP JOB APPLICATIONS AND EMPLOYEE RECORD
    ...
    await setUpJobApplications(formDataList);
