...
module.exports.main = function (ffCollection, vvClient, response) {
  /*Script Name:   LibUserCreate
     ...
     Parameters: The following represent variables passed into the function from an array:
                ...
                Send Email - (String, Required) Pass in 'Standard', 'Custom', 'Both' or 'None'.
    ...
    Return Array:  The following represents the array of information returned to the calling function.  This is a standardized response.
     ...
     04/01/2021 - Alyssa Carpenter: Add the option to pass in Send Email as 'None' which will not send any email.
     07/20/2022 - Fernando Culell: Add a modification on the query parameter for getGroups in oreder to retrieve only th system groups that are needed.
     */
  ...
  return result
    .then(function () {
      ...
      if (!sendEmail || !sendEmail.value) {
        ...
        if (
          ...
          errors.push(
            "The Send Email parameter must be 'Standard', 'Custom', 'Both' or 'None'."
          );
        }
      }
      ...
      if (!password || !password.value || password.value.trim() == "") {
        ...
        if (password.length < minPasswordLength) {
          errors.push(
            "The password must be at least " + minPasswordLength + " long. "
          );
        }
      ...
      if (createFolder) {
        if (
          ...
          if (
            ...
            errors.push(
              "The Folder Permissions parameter must be Viewer, Editor, or Owner."
            );
          }
        }
      } else {
      ...
      if (sendEmail == "Custom" || sendEmail == "Both") {
        ...
        if (!bodyField || !bodyField.value) {
          ...
          if (sendEmail == "Custom") {
            if (
              bodyField.indexOf("[Username]") == -1 ||
              bodyField.indexOf("[Password]") == -1
            ) {
              errors.push(
                "The Email Body must include [Username] and [Password]."
              );
            }
          }
        }
      ...
      if (!relateToRecords || !relateToRecords.value) {
        ...
        if (Array.isArray(relateToRecords) == false) {
          errors.push(
            "The Related Records parameter must be an array when it is provided."
          );
        }
      }
          ...
          if (userData.meta.status == 200) {
            if (userData.data.length == 1) {
              ...
              if (userEnabled == 1) {
                ...
                throw new Error(
                  "A duplicate user was found, but the process was unable to determine if the user account is currently enabled or disabled. " +
              ...
              logger.info(
                "Searched for existing users with ID " +
                  userID +
                  ". None found. Continuing the process."
              );
            } else {
              //Many users found. Invalid state.
              throw new Error(
                "More than one user was found for ID: " +
                  userID +
                  ". This is an invalid state. Please notify a system administrator."
              );
            }
          } else if (userData.meta.status == 404) {
            throw new Error(
              "The custom query to find users was not found. Please ensure that a query named " +
                userQueryName +
                " has been configured in the default queries area."
            );
          } else {
            throw new Error(
        ...
        .then(function (siteResp) {
          ...
          if (siteData.meta.status === 200) {
            if (siteData.data.length === 0) {
              ...
              throw new Error(
                "The call to search for the site name " +
                  siteName +
                  " returned with more than one result. This is an invalid state. Please contact a system administrator."
              );
            }
          } else {
            throw new Error(
    ...
    .then(function () {
      if (groupsPassed) {
        ...
        let groupArrayUntrimmed = groupList.split(",");
        let groupArray = arrayTrimmer(groupArrayUntrimmed);

        //Create the string with the values needed for the query.
        let tempContent = "";
        groupArray.forEach((group) => {
          tempContent += `'${group}',`;
        });
        let valueString = tempContent.slice(0, -1);

        //Check if the groups passed in by the user exist.
        var groupParam = {};
        groupParam.q = `[name] IN (${valueString})`;
        groupParam.fields = "id,name,description";

        //Function to get the required system groups
        return vvClient.groups.getGroups(groupParam).then(function (groupResp) {
          ...
          if (groupData.meta.status === 200) {
            if (groupData.data.length > 0) {
              ...
              if (groupErrors != "") {
                throw new Error(
                  "At least one group was not found to exist. " + groupErrors
                );
              } else {
                logger.info(
                  "All groups were found to exist. Continuing the process."
                );
              }
            } else {
              throw new Error(
                "No user permission groups were found to exist in the system. Please contact a system administrator."
              );
            }
          } else {
            throw new Error(
              "The call to get existing group names returned with an error."
            );
          }
        });
      }
    ...
    .then(function () {
      ...
      if (sendEmail == "Standard" || sendEmail == "Both") {
      ...
      return vvClient.users
        ...
        .then(function (userResp) {
          if (userResp.meta.status === 200) {
            ...
            throw new Error(
              "The call to create the user returned with an error."
            );
          }
        });
    })
        ...
        groupIdArray.forEach(function (groupItem) {
          addGroupProcess = addGroupProcess.then(function () {
            return vvClient.groups
              .addUserToGroup({}, groupItem.id, userGUID)
              .then(function (addResp) {
                ...
                if (addData.meta.status !== 201) {
                  ...
                  logger.info(
                    "Error adding user to group " + groupItem.name + "."
                  );
                  errors.push(
                    "Error adding user to group " + groupItem.name + "."
                  );
    ...
    .then(function () {
      ...
      if (sendEmail == "Custom" || sendEmail == "Both") {
        ...
        emailData.body = bodyWithoutPassword.replace(
          "[Password]",
          userPassword
        );
    ...
    .then(function () {
      ...
      if (createCommLog && (sendEmail == "Custom" || sendEmail == "Both")) {
        ...
        return vvClient.forms
          ...
          .then(function (postResp) {
            if (postResp.meta.status === 201 || postResp.meta.status === 200) {
              ...
              if (relateToRecords.length > 0) {
                ...
                relateToRecords.forEach(function (relatedForm) {
                  relateProcessResult = relateProcessResult.then(function () {
                    return vvClient.forms
                      .relateFormByDocId(commLogRevisionId, relatedForm)
                      .then(function (relateResp) {
                        var relatedResp = JSON.parse(relateResp);
                        if (
                          relatedResp.meta.status === 200 ||
                          relatedResp.meta.status === 404
                        ) {
                          logger.info(
                            "Communications Log related to form " +
                              relatedForm +
                              " successfully."
                          );
                        } else {
                          logger.info(
                            "Call to relate the Communications Log to form " +
                              relatedForm +
                              " returned with an error."
                          );
                          errors.push(
              ...
              errors.push(
                "Call to post Communications Log form returned with an error. The server returned a status of " +
          ...
          .then(function (folderResp) {
            ...
            if (folderData.meta.status === 200) {
              if (folderData.data.length > 0) {
                ...
                logger.info(
                  "The call to find a folder at " +
                    folderPath +
                    " returned with no duplicates. Continuing the process."
                );
              }
              ...
              logger.info(
                "The call to find a folder at " +
                  folderPath +
                  " returned with a 403. Assuming no duplicates and continuing the process."
              );
            } else {
              logger.info(
                "The call to find a folder at " +
    ...
    .then(function () {
      ...
      if (createFolder && folderId == "") {
        ...
        return vvClient.library
          ...
          .then(function (createFolderResp) {
            if (createFolderResp.meta.status === 200) {
              logger.info("Folder created successfully.");
              folderId = createFolderResp.data.id;
            } else {
              errors.push(
                "User was created but call to create folder at " +
                  folderPath +
                  " returned with an error."
              );
            }
    ...
    .then(function () {
      ...
      if (securityLevel != "" && folderId != "") {
        ...
        return vvClient.library
          ...
          .then(function (folderSecurityResp) {
            if (folderSecurityResp.meta.status === 200) {
              logger.info("Security permissions successfully added to folder");
            } else {
              errors.push(
                "The folder was created, but an error was returned when attempting to add user security permissions to the folder."
              );
            }
          });
      }
