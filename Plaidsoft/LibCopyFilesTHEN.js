...
module.exports.main = async function (ffCollection, vvClient, response) {
  ...
  try {
    ...
    if (permissionGroups) {
      // 4.Gets folder's current permissions settings
      await getFolderPermissionMembersIDs(folderId)
        .then((folderPermissionsIDs) => {
          // 5.Deletes current members with permissions
          // This is because we can only ADD new permissions
          if (folderPermissionsIDs.length > 0) {
            return deleteFolderPermissions(folderId, folderPermissionsIDs);
          }
        })
        .then(() => {
          // 6.Sets new permissions to the folder
          return setGroupsFolderSecurity(folderId, permissionGroups);
        });
    }

    // 7.Gets the original document ids
    await getDocumentsData(sourcePath)
      .then((sourceDocsData) => {
        // 8.Copies the documents
        let copyPromises = [];

        for (const doc of sourceDocsData) {
          copyPromises.push(copyFile(doc, folderId));
        }
