...
module.exports.main = async function (vvClient, response, token) {
    ...
    try {
        ...
        const queryName1 = "Complaints High Severity Query";
        const formTemplateName = "Complaints";
        shortDescription = "Custom Query retrieving forms with a value greater than 7 on its Severity data field and not labeled as High Severity yet";
        ...
        await Promise.all(
            customQueryResp.data.map(async (element) => {
                let dataUpdate = {
                    Description: `[High Severity] ${element.description}`,
                };
